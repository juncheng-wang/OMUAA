...
par.beta = 0;                                                               % step size
par.lambda = 5e-4;                                                          % power scaling
par.gamma = 2e-2;                                                           % step size
par.kappa = 0;                                                              % regularization
par.Sp = 0;                                                                 % sparsification level
...
%% OMUAA-DR
function [xtc,xtnc,lambda] = Func_OMUAADR(Data,Label,H,Z,sys,par)
    N = sys.N;
    T = par.T;
    D = sys.D;
    d = sys.d;
    ...
    for t = 1:T-1
        ...
        for c = 1:C
            for n = 1:N
                ...
                H_tnc = H{t+1}{c}(n,:);
                e_tnc = (1./abs(H_tnc).^2).';
                A_tnc = 2*alpha*ones(d,1)+ gamma*(Qtnc{t}(n,c)+gamma*gtn{t}(n,c))*GD_tnc_g(idx,n);
                B_tnc = 2*alpha*xtc{t}{c} + 2*beta*xtnc{t}{n}{c} - GD_tnc_f(idx,n);
                xtnc{t+1}{n}{c} = (1./A_tnc) .* B_tnc;
                [~,idx] = mink(abs(xtnc{t+1}{n}{c}),round(784*Sp));
                xtnc{t+1}{n}{c}(idx) = 0;
                xtc{t+1}{c} =  xtc{t+1}{c} + w(t,n) * xtnc{t+1}{n}{c};
                gtnc{t+1}(n,c) = lambda(t+1)^2 * w(t,n)^2 * xtnc{t+1}{n}{c}.' * diag(e_tnc) * xtnc{t+1}{n}{c} - Pnc_bar;    
                Qtnc{t+1}(n,c) = max(-gamma*gtnc{t+1}(n,c) , Qtnc{t}(n,c) + gamma*gtnc{t+1}(n,c));
            end
            if par.add_noise == 1
                xtc{t+1}{c} = xtc{t+1}{c} + 1/lambda(t+1) * Z{t+1}{c};
            end
        ...
        time = toc;
        fprintf('OMUAA-DR, t:%d \n',t);
    end
end

%% OTA-FL
function [xtc,xtnc,lambda] = Func_OTAFL(Data,Label,H,Z,sys,par)
    T = par.T;
    N = sys.N;
    D = sys.D;
    d = sys.d;
    ...
    for t = 1:T-1
        ...
        for n = 1:N
            for i = 1:D(t,n)
                ...
                h_tni = zeros(1,C);
                for c = 1:C
                    h_tni(c) = exp( d_tni.' * xtc{t}{c});
                end
                hsum_tn = sum(h_tni);
                for c = 1:C
                    idx = d*(c-1)+1:d*c;
                    GD_tnc(idx,n) = GD_tnc(idx,n) - 1/D(t,n) * ( (b_tni == c-1) - h_tni(c)/hsum_tn ) * d_tni + 2*kappa *xtc{t}{c} ;
                end
            end
        end
        for c = 1:C
            for n = 1:N
                idx = d*(c-1)+1:d*c;
                xtnc{t+1}{n}{c} = xtc{t}{c} - 1/(2*alpha) * GD_tnc(idx,n);
            end
        end
        P_bar = Func_dBm2W(par.Pnbar_dBm) * N * par.slot;
        if t == 1
            Power(1) = 0;
        end
        Pt = 0;
        for n = 1:sys.N
            for c = 1:C
                H_tnc = H{t+1}{c}(n,:);
                e_tnc = (1./abs(H_tnc).^2).';
                Pt = Pt + xtnc{t+1}{n}{c}.' * w(t,n)^2 * diag(e_tnc) * xtnc{t+1}{n}{c};
            end
        end
        lambda(t+1) = sqrt(P_bar/Pt);
        for c = 1:C
            xtc{t+1}{c} = zeros(d,1);
            for n = 1:N
                xtc{t+1}{c} =  xtc{t+1}{c} + w(t,n) * xtnc{t+1}{n}{c};
